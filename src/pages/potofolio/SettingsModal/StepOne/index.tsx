...
const StepOne = (props: StepOneType) => {
  ...
  const changeEmail = async () => {
    try {
      ...
      onSaveEmail(form.getFieldValue('email'), emailCode.data.emailCode);
      onchangeStep(2);
    } catch (error) {
      Message.warning({
  ...
  return (
    <div className={styles.stepOne}>
      <div className={styles.emall}>
        <img src="/svg/邮箱.svg" alt="" />
      </div>
      <div className={styles.emallTitle}>Change email</div>
      <div style={{ marginTop: '30px' }} className={styles.formContent}>
        <Form form={form}>
          <FormItem
            field="email"
            rules={[
      ...
      <div className={styles.changeEmail}>
        <Button style={{ width: '100%', height: '48px' }} onClick={changeEmail}>
          Continue
        </Button>
      </div>
    </div>
